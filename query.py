...
def prometheus_request(request_type, query, start=None, end=None, step=None, logging_level=0, source_type="node_hybrid", ip='localhost'):
  if (start!=None and end!=None and step!=None):
    string = 'http://' + ip + ':9090/api/v1/' + request_type + '?query=' + query + '&start='+start+'&end='+end + '&step=' + step
  else:
    string = 'http://' + ip + ':9090/api/v1/' + request_type + '?query=' + query 
  print string
...
# we get all the prometheus IP addresses using mesos dns
def getPrometheusIP():
  addresses = request("10.10.1.71", "prometheus")
  return addresses
...
def sendRequest(query):
  ...
  prometheus_ips = getPrometheusIP()
  for ip in prometheus_ips:
    prometheus_request('query', query, start, end,logging_level=1, ip=ip)
